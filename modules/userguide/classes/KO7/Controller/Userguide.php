<?php
/**
 * KO7 user guide and api browser.
 *
...
abstract class KO7_Controller_Userguide extends Controller_Template {
	...
	public function before()
	{
		...
		$this->template->show_comments = KO7::$config->load('userguide.show_comments');
	}
	...
	public function error($message)
	{
		...
		if ($module = $this->request->param('module') AND $menu = $this->file($module.'/menu') AND KO7::$config->load('userguide.modules.'.$module.'.enabled'))
		{
			...
			$this->template->breadcrumb = [
				...
				$this->guide->uri(['module' => $module]) => KO7::$config->load('userguide.modules.'.$module.'.name'),
				'Error'
			];
		}
	...
	public function action_docs()
	{
		...
		if ( ! KO7::$config->load('userguide.modules.'.$module.'.enabled'))
		{
		...
		$this->template->title = ($page == 'index')
			? KO7::$config->load('userguide.modules.'.$module.'.name')
			: $this->title($page);
		...
		$breadcrumb[$this->guide->uri(['module' => $module])] = KO7::$config->load('userguide.modules.'.$module.'.name');
	...
	public function action_media()
	{
		...
		if ($file = KO7::find_file('media/guide', $file, $ext))
		{
	...
	public function after()
	{
		if ($this->auto_render)
		{
			...
			// Add languages
			$this->template->translations = KO7::message('userguide', 'translations');
		}
	...
	public function file($page)
	{
		...
		return KO7::find_file('guide', $page, 'md');
	}
	...
	// Get the list of modules from the config, and reverses it so it displays in the order the modules are added, but move KO7 to the top.
	protected function _modules()
	{
		$modules = array_reverse(KO7::$config->load('userguide.modules'));

		if (isset($modules['koseven']))
		{
			$koseven = $modules['koseven'];
			unset($modules['koseven']);
			$modules = array_merge(['koseven' => $koseven], $modules);
		}

		// Remove modules that have been disabled via config
		foreach ($modules as $key => $value)
		{
			if ( ! KO7::$config->load('userguide.modules.'.$key.'.enabled'))
			{
				unset($modules[$key]);
			}
		}
